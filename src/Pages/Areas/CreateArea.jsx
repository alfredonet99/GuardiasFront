...
export default function CreateArea() {
	...
	const rules = useMemo(
		() => ({ name: { required: true, message: "Ingresa un nombre de área" } }),
		[],
	);
	...
	const handleCreateArea = async () => {
		...
		try {
			...
			if (status === 422) {
				...
				showMessage(
					error.response?.data?.message || "No tienes permiso.",
					"error",
				);
			} else {
		...
		<div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
			...
			<section className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-6 max-w-3xl mx-auto">
				<div className="space-y-5">
					<div>
						<label htmlFor="area_name" className="font-semibold text-sm">
							Nombre del Area
						</label>
						<input
							type="text"
