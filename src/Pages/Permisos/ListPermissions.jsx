...
export default function ListPermissions() {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      setLoading(true);
      try {
        const res = await privateInstance.get("/permissions", { params: { search, page,}, });
        setPermissions(res.data.data || []);
        setMeta({ last_page: res.data.last_page ?? 1, total: res.data.total ?? (res.data.data?.length || 0), per_page: res.data.per_page ?? 0, });
      } catch (err) {
      ...
      <section className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
          <div className="relative w-full md:max-w-sm">
            <SearchInputLong value={query} onChange={(e) => setQuery(e.target.value)} onDebouncedChange={(val) => { if ((val ?? "").trim() === (search ?? "").trim()) return; setSearch(val);  setPage(1); }} placeholder="Buscar permisos..."/>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400"> {meta.total} permiso(s) </span>
        </div>

        <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800">
          {loading ? (
            <div className="p-6 text-center text-slate-500 dark:text-slate-400"> Cargando permisos... </div>
          ) : permissions.length === 0 ? (
            <div className="p-6 text-center text-slate-500 dark:text-slate-400"> No se encontraron permisos. </div>
          ) : (
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {permissions.map((p, idx) => (
                  <tr key={p.id} className={`${ idx % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-slate-50 dark:bg-slate-900/60"} hover:bg-slate-100 dark:hover:bg-slate-800/70 transition`}>
