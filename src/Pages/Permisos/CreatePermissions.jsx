...
export default function CreatePermissions() {
	...
	const modules = useMemo(() => {
		const all = routeConfig.filter((r) => r.module).map((r) => r.module);
	...
	const [crudSelected, setCrudSelected] = useState({
		browse: true,
		create: true,
		edit: true,
		delete: true,
	...
	const handleCreateCrud = async () => {
		const rules = {
			...
			crudSelected: {
				required: true,
				message: "Selecciona al menos una acción CRUD.",
			},
		};
		...
		const crudPayload = selectedCRUD.map((action) => ({
			action,
		...
		try {
			...
			if (err.response?.status === 422) {
				...
				if (Array.isArray(existing) && existing.length > 0) {
					showMessage(
						`Algunos permisos ya existen: ${existing.join(", ")}`,
						"error",
					);
				}
			} else {
		...
		<div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
			...
			<div className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-5 max-w-3xl mx-auto">
				<div className="flex border-b border-slate-300 dark:border-slate-700 mb-6">
					<button
						type="button"
						className={`px-4 py-2 font-semibold flex items-center gap-2 border-b-2 transition ${tab === "single" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
					...
					<button
						type="button"
						className={`px-4 py-2 font-semibold flex items-center gap-2 border-b-2 transition ${tab === "crud" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
						onClick={() => setTab("crud")}
					>
				...
				{tab === "single" && (
					<div className="space-y-4 animate-fadeIn">
						<div>
							...
							<FieldError
								message={localErrors.moduleSelected}
								resetKey={errorKey}
							/>
						</div>

						<div>
							<label htmlFor="permission_name" className="font-medium">
								Nombre del Permiso
							</label>
							<input
								type="text"
								...
								placeholder="Ejemplo: export, approve, validate"
								className="mt-1 w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-blue-600 outline-none"
							/>
						...
						<div>
							<label htmlFor="permission_description" className="font-medium">
				...
				{tab === "crud" && (
					<div className="space-y-4 animate-fadeIn">
						<div>
							...
							<select
								value={moduleCRUD}
								onChange={(e) => setModuleCRUD(e.target.value)}
								className="mt-1 w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 
							...
							<FieldError
								message={localErrors.moduleCRUD}
								resetKey={errorKey}
							/>
						</div>

						<div>
							<label htmlFor="permission_selectcrud" className="font-medium">
								Selecciona los permisos CRUD
							</label>
							<div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
								{Object.entries(crudSelected).map(([key, value]) => (
									<label
										key={key}
										className="flex items-center gap-2 cursor-pointer bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700"
									>
										<input
											type="checkbox"
											checked={value}
											onChange={() => toggleCrud(key)}
										/>
										{key.toUpperCase()}
									</label>
								))}
							...
							<FieldError
								message={localErrors.crudSelected}
								resetKey={errorKey}
							/>
						</div>
						<FlashMessage message={message} />
						<button
							type="button"
							onClick={handleCreateCrud}
							className="w-full py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition"
						>
							{" "}
							Crear Permisos del CRUD{" "}
						</button>
					</div>
				)}
			</div>
