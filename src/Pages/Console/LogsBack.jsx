...
import { FiArrowLeft, FiRefreshCw, FiSearch, FiTerminal } from "react-icons/fi";
...
export default function SystemConsoleTablePage() {
	...
	const {
		...
		loading,
		reload,
	} = usePagination("/system/logs", { perPage: 50 });
			...
			<header className="px-6 py-4 border-b border-slate-300 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 backdrop-blur-md flex justify-end">
				<div className="flex items-center gap-4">
					<button
						...
						className="flex items-center gap-2 px-3 py-1.5 bg-blue-200/40 text-blue-700 border border-blue-400/40 dark:bg-blue-700/20 dark:text-blue-300 rounded-md hover:bg-blue-300/50 dark:hover:bg-blue-700/30 transition"
					>
					...
					<button
						...
						className="flex items-center gap-2 px-3 py-1.5 bg-slate-200/60 border border-slate-400 dark:bg-slate-800/50 dark:border-slate-700 rounded-md hover:bg-slate-300 dark:hover:bg-slate-700 transition"
					>
						<FiArrowLeft /> Volver
					</button>
				</div>
								...
								<React.Fragment key={log.id}>
									...
									>
										<td className="py-2 px-2">
											<span
												className={`px-2 py-1 rounded-md border text-xs font-semibold ${getTagStyle(log.type)}`}
											>
												{log.type}
											</span>
										</td>

										<td className="py-2 px-2 flex items-center justify-between max-w-[400px]">
									...
									{expanded === log.id && (
										<tr>
											<td
												colSpan={3}
												className="bg-slate-100 dark:bg-slate-900 px-4 py-3 border-b border-slate-300 dark:border-slate-800"
											>
