...
export default function EditClientVeeam() {
	...
	}, [clearMessage, showMessage, handleChange]);

	const safeClearMessage = useCallback(() => clearMessageRef.current?.(), []);
	const safeShowMessage = useCallback(
		(msg, type) => showMessageRef.current?.(msg, type),
		[],
	);
	const safeHandleChange = useCallback((v) => handleChangeRef.current?.(v), []);
		...
		const load = async () => {
			...
			try {
				const res = await privateInstance.get(
					`operaciones/cliente-veeam/editar/${id}`,
				);
				if (!alive) return;

				const c = res.data?.data || res.data?.cliente || res.data || {};

				const currentNum = (c.numCV || "").toString();
				setnumCV(currentNum);
				safeHandleChange(currentNum);

				setnameCv(c.nameCV || "");
				setApp(c.app ? String(c.app) : "");

				const b = (c.backup || "").toString().trim();
				const m = b.match(/^(\d+(?:\.\d+)?)\s*(GB|TB)$/i);
				if (m) {
					setStorageValue(m[1]);
					setUnit(m[2].toUpperCase());
				} else {
					setStorageValue(b.replace(/\s+(GB|TB)$/i, "").trim());
					const unitMatch = b.match(/\b(GB|TB)\b/i);
					setUnit(unitMatch ? unitMatch[1].toUpperCase() : "GB");
				}

				setJobs(Number.isFinite(Number(c.jobs)) ? Number(c.jobs) : 0);
				...
				if (status === 404) {
					safeShowMessage("El cliente no existe o fue eliminado.", "error");
				} else {
					safeShowMessage(
	...
	}, [id, safeClearMessage, safeHandleChange, safeShowMessage]);
	...
	const handleUpdate = async () => {
		...
		setSaving(true);
		try {
			await privateInstance.put(`operaciones/cliente-veeam/update/${id}`, {
			...
			if (status === 422) {
				...
				safeShowMessage(first || "Revisa los campos del formulario.", "error");
			} else if (status === 409) {
				safeShowMessage(
					error.response?.data?.message ||
	...
	return (
		<div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
			<header className="flex items-center justify-between mb-6">
				<h1 className="text-2xl font-bold mx-1">Editar Cliente Veeam</h1>
