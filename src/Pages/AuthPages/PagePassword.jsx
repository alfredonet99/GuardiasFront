...
export default function PasswordPage() {
  ...
  if (validating) {
    return (
      <div className="min-h-screen flex items-center justify-center text-slate-700 dark:text-slate-200"> Validando enlace… </div>
    );
  }
        ...
        <div className="p-8">
          <div className="flex justify-center mb-6">
            <div className="w-14 h-14 flex items-center justify-center rounded-full border border-slate-400 dark:border-slate-500 bg-slate-200/60 dark:bg-slate-800/40 transition">
              <span className="text-3xl text-slate-700 dark:text-slate-300"> 🔑 </span>
            </div>
          </div>

          <h2 className="text-center text-xl font-semibold text-slate-800 dark:text-white"> Restablecer contraseña </h2>
          <p className="mt-2 text-center text-sm text-slate-600 dark:text-slate-400"> Ingresa tu nueva contraseña para continuar. </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <input type="password" placeholder="Confirmar contraseña" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} className="w-full px-3 py-2 rounded-md bg-white dark:bg-slate-800
            ...
            <button type="submit" disabled={loading} className="w-full py-2 rounded-md bg-blue-600 hover:bg-blue-700 text-white font-medium shadow-sm">
