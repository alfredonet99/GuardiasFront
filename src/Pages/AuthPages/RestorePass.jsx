...
export default function RecoverPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cooldown > 0) {
      setError("Ya hemos enviado una liga a tu correo <br> vuelve a intentarlo en 1 minuto.");
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-950 transition-colors px-4">
      <div className="w-full max-w-md bg-white/70 dark:bg-slate-900/50 border border-slate-300 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden backdrop-blur-sm transition-colors">
        <div className="p-8">
          <div className="flex justify-center mb-6">
            <div className="w-14 h-14 flex items-center justify-center rounded-full border border-slate-400 dark:border-slate-500 bg-slate-200/50 dark:bg-slate-800/40 transition-colors">
              <span className="text-3xl text-slate-600 dark:text-slate-300"> 🔒 </span>
            </div>
          </div>

          <h2 className="text-center text-xl font-semibold text-slate-800 dark:text-white"> ¿Tienes problemas para iniciar sesión? </h2>

          <p className="mt-2 text-center text-sm text-slate-600 dark:text-slate-400"> Ingresa tu correo y te enviaremos un enlace para recuperar tu cuenta. </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full py-2 rounded-md bg-blue-600 hover:bg-blue-700 text-white font-medium shadow-sm transition">
        ...
        <div className="bg-slate-200 dark:bg-slate-900 px-6 py-4 text-center border-t border-slate-300 dark:border-slate-800 transition-colors">
          <NavLink to="/login" className="text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white transition-colors">
