...
export default function EditProfile() {
  ...
  const doPasswordsMatch = !passwords.confirm || passwords.new === passwords.confirm;
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (pendingAvatarFile) { avatarUrl = await uploadAvatar( pendingAvatarFile, user.id, user.avatar ); }

      const { data } = await privateInstance.post("/profile/update", { name, avatar: avatarUrl, });
      ...
      if (error.response?.data?.error) { showMessage(error.response.data.error, "error");
      } else if (error.response?.data?.message) { showMessage( "No hemos podido procesar tu solicitud");
      } else { showMessage("Error al guardar el perfil", "error");
      }
    ...
    <div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-6 max-w-3xl mx-auto space-y-8">
        ...
        <section className="flex items-center gap-6">
          <img src={preview} className="w-24 h-24 rounded-full object-cover border border-slate-300 dark:border-slate-700"/>

          <label className="cursor-pointer">
            <input type="file" hidden accept="image/*" onChange={handleAvatarChange} disabled={loading}/>
            <span className="px-4 py-2 rounded-lg bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition"> Cambiar avatar </span>
          </label>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div>
            ...
            <input value={role || "Sin Rol"} disabled
              className="mt-1 w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-500"
            />
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="relative">
            ...
            <input type={currentType} value={passwords.current} onChange={(e) => setPasswords({ ...passwords, current: e.target.value })}
            ...
            <div className="absolute mt-3 right-3 top-1/2 -translate-y-1/2"> {CurrentToggle} </div>
          </div>

          <div className="space-y-1">
            <div className="relative">
              <label className="font-medium">Nueva Contraseña</label>
              <input type={newType} value={passwords.new} onChange={(e) => setPasswords({ ...passwords, new: e.target.value })}
            ...
            {!isPasswordValid && (
              <p className="text-sm text-red-600 dark:text-red-400"> La contraseña debe tener al menos 6 caracteres. </p>
            )}
          </div>
          ...
          <div className="space-y-1">
            <div className="relative">
              ...
              <input type={confirmType} value={passwords.confirm} onChange={(e) => setPasswords({ ...passwords, confirm: e.target.value })}
                className={`mt-1 w-full px-4 py-2 pr-10 rounded-lg border 
                  ${!doPasswordsMatch
                    ? "border-red-500 focus:ring-red-500"
                    : "border-slate-300 dark:border-slate-600 focus:ring-blue-600"}
          ...
          <div className="rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 px-4 py-3 text-sm text-blue-700 dark:text-blue-300">
            ...
            <p> Al actualizar tu contraseña, la sesión actual se mantiene activa. <br /> La nueva contraseña se aplicará la próxima vez que inicies sesión. </p>
          </div>

          <FlashMessage message={message} />
        </section>

        <button onClick={handleSaveProfile} disabled={loading} className="w-full py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition">
