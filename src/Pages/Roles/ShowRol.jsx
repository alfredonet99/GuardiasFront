...
export default function ShowRole() {
  ...
  if (loading) {
    return (
      <div className="p-6 text-center text-slate-500 dark:text-slate-300"> Cargando información del rol... </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
      ...
      <section className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-6">
        ...
        <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-3"> Permisos asignados </h2>

        {permissions.length === 0 ? (
          <div className="text-sm text-slate-500 dark:text-slate-400"> Este rol no tiene permisos asignados. </div>
        ) : (
                ...
                <div key={module} className="border border-slate-200 dark:border-slate-800 rounded-lg p-4 bg-slate-50/60 dark:bg-slate-900/60">
                  ...
                  {isOpen && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                      {perms.map((perm) => (
                        <div key={perm.id} className="flex flex-col gap-1 bg-white dark:bg-slate-800 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700">
                          <div className="text-sm font-semibold break-all"> {perm.name} </div>
                          {perm.description && (
