...
export default function CreateRole() {
  ...
  const handleCreate = async () => {
    const rules = {roleName : {required: true, message: "Ingresa un nombre de rol valido"},  assigned: { required: true, message: "Selecciona al menos un permiso." },}
    ...
    try {
      ...
      if (err.response?.status === 422) {
        ...
        if (errs) {
          ...
          showMessage(err.response.data?.message || "El nombre del rol ya existe.", "error");
        }
      } else {
  ...
  return (
    <div className="min-h-screen w-full px-6 py-6 bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200">
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2 mx-1">Crear Rol</h1>
      ...
      <section className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-6">
        <div className="mb-6">
          ...
          <input type="text" value={roleName} onChange={(e) => setRoleName(e.target.value)} placeholder="Ej. supervisor_operaciones"
          ...
          {Object.entries(grouped).map(([module, perms]) => {
            ...
            return (
              <div key={module} className="border border-slate-200 dark:border-slate-800 rounded-lg p-4 bg-slate-50/60 dark:bg-slate-900/60">
                <button type="button" onClick={() => toggleModule(module)} className="w-full flex items-center justify-between px-3 py-2 rounded-md border border-slate-300/70 dark:border-slate-700
                ...
                {isOpen && (
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
                      {perms.map((perm) => (
                        <label key={perm.id} className="flex items-start gap-2 cursor-pointer bg-white dark:bg-slate-800 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
        ...
        <div className="mt-6 flex justify-end">
          <button onClick={handleCreate} disabled={saving} className={`px-5 py-2 rounded-lg text-white transition shadow-sm ${ saving ? "bg-blue-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}>
