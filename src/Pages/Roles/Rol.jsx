	...
	return (
		<div className="min-h-screen w-full bg-slate-100 dark:bg-slate-950 px-6 py-6 text-slate-800 dark:text-slate-200">
			<header className="flex items-center justify-between mb-6">
				<h1 className="text-2xl font-bold flex items-center gap-2 px-2 text-slate-800 dark:text-slate-100">
					{" "}
					Gestión de Roles{" "}
				</h1>
				{!loadingMe && isAdmin && (
					<IconCreate to="/admin/roles/crear" label="Rol" />
				)}
			</header>
											...
											<div className="flex items-center justify-center gap-2">
												...
												{!loadingMe && isAdmin && (
													<IconDelete
														onClick={() =>
															openModal({
																message: `¿Quieres eliminar el rol "${role.name}"?, \nEsta acción no se podra revertir`,
