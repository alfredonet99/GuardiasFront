...
export default function EditUser() {
	...
	useEffect(() => {
		...
		if (!isAdmin && targetIsAdmin && !denied) {
			showMessage(
				"No tienes permiso para editar un usuario Administrador.",
				"error",
			);
			setDenied(true);
		}
			...
			<section className="bg-white dark:bg-slate-900 rounded-xl shadow border border-slate-200 dark:border-slate-800 p-6 max-w-3xl mx-auto">
				<div className="space-y-5">
					...
					<div>
						...
						<input
							type="email"
							value={email}
							onBlur={() => isAdmin && markTouched("email")}
					...
					<div>
						...
						<select
							...
							onBlur={() => markTouched("roleSelected")}
							className="mt-1 w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-blue-600 outline-none"
						>
						...
						{isTouched("roleSelected") && (
							<FieldError
								message={localErrors.roleSelected}
								resetKey={errorKey}
							/>
						)}
					</div>
					...
					<div className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/60 px-4 py-3">
						...
						<button
							...
							className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${activo ? "bg-blue-600" : "bg-slate-400"}`}
							aria-pressed={activo}
						>
							<span
								className={`inline-block h-5 w-5 transform rounded-full bg-white transition ${activo ? "translate-x-5" : "translate-x-1"}`}
							/>
						</button>
					</div>
					...
					<div className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/60 px-4 py-3">
						<div>
							...
							<div className="text-xs text-slate-500 dark:text-slate-400">
								{" "}
								Actívalo solo si deseas actualizar la contraseña.{" "}
							</div>
						</div>
						...
						<button
							...
							className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${changePassword ? "bg-blue-600" : "bg-slate-400"}`}
							aria-pressed={changePassword}
						>
							<span
								className={`inline-block h-5 w-5 transform rounded-full bg-white transition ${changePassword ? "translate-x-5" : "translate-x-1"}`}
							/>
						</button>
					</div>
				...
				<div className="mt-6 flex justify-end">
					<button
						...
						className="px-5 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition shadow-sm disabled:opacity-60"
					>
						Guardar cambios
					</button>
				</div>
