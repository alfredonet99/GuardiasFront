...
export default function ShowUser() {
	...
	const { loading: loadingMe } = useAuthMe();
	const [_targetIsAdmin, setTargetIsAdmin] = useState(null);
	...
	useEffect(() => {
		...
		const load = async () => {
			try {
				...
				const r = Array.isArray(data?.roles)
					? data.roles
				...
				const dp = Array.isArray(data?.direct_permissions)
					? data.direct_permissions
					: Array.isArray(data?.user?.direct_permissions)
								...
								{Object.entries(groupedDirect).map(([module, perms]) => (
									...
									>
										<div className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-300">
											{module}{" "}
											<span className="opacity-70">({perms.length})</span>
										</div>
