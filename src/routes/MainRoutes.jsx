...
export default function MainRoutes() {
    ...
    const tokenExpiredByTime = expStr ? Date.now() >= parseInt(expStr, 10) : false;
    ...
    return (
        <Routes>
            <Route path="/login" element={hasValidToken ? <Navigate to="/inicio" replace /> : <LoginPage />} />
            ...
            <Route path="/" element={hasValidToken ? <Navigate to="/inicio" replace /> : <LoginPage />}/>
            ...
            <Route element={ < MainLayout />}>
                {routeConfig.map(({ path, component, module, permissions }) => {
                    ...
                    return <Route key={path} path={path} element={ <RouteGuard module={module} required={permissions}> <PageComponent /> </RouteGuard>} />;
                })}
            </Route>
