...
export default function useOptimisticToggle({
  setItems, getId = (x) => x.id, getBool, setBool, buildUrl,              
  buildBody = (next) => ({ activo: next }), readBoolFromResponse, client,                
}) {
  const [loadingId, setLoadingId] = useState(null);

  const toggle = async (item) => {
    const id = getId(item);
    const next = !Boolean(getBool(item));

    setItems((prev) => prev.map((x) => (getId(x) === id ? setBool(x, next) : x)));
    setLoadingId(id);
