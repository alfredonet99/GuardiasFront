...
import { IconCreate, IconDelete, IconEdit, IconShow } from "../../icons/Crud/exportCrud";
...
export default function TableUsers() {
  ...
  const isUserAdmin = (u) => (u.roles || []).some((r) => r.name === "Administrador");
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      setLoading(true);
      try {
        const res = await privateInstance.get("/users", { params: { search, page, per_page: 5 },});
  ...
  const toggleUserStatus = async (u) => {
    ...
    try {
      ...
      const activeFromApi = Boolean(updated.activo ?? updated.Activo ?? nextActive);
    ...
    <div className="mt-10 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div className="relative w-full md:max-w-sm">
          <SearchInputLong value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar usuarios por nombre o correo..."
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
        {loading ? (
          <div className="p-6 text-center text-slate-500 dark:text-slate-400"> Cargando usuarios... </div>
        ) : users.length === 0 ? (
          <div className="p-6 text-center text-slate-500 dark:text-slate-400"> No se encontraron usuarios </div>
        ) : (
          <table className="min-w-full text-left">
            ...
            <tbody className="text-slate-700 dark:text-slate-300 text-sm">
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} className="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                    <td className="px-4 py-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-300 dark:bg-slate-700 flex items-center justify-center">
                        {u.avatar ? (
                          <img src={u.avatar} alt="Foto perfil" className="w-full h-full object-cover"/>
                        ) : (
                          <span className="text-xs text-slate-600 dark:text-slate-400"> Sin foto </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-3 py-1 rounded-full text-xs ${u.active ? "bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-300": "bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300"}`}>
                        {u.active ? "Activo" : "Inactivo"}
                      </span>
                    </td>

                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300"> {formatDateTime(u.last_login_at)} </td>


                    <td className="px-4 py-3">
                      <div className="flex justify-center gap-2">
                       {(isAdmin || !targetIsAdmin) && (<IconEdit to={`/admin/users/${u.id}/editar`} />)}

                        <IconShow to={`/admin/users/${u.id}/ver`} />
                        
                        {!isMeRow && (
                          <ToggleUserStatusButton active={u.active} label={u.active ? "Desactivar" : "Activar"} loading={loadingUserId === u.id} onToggle={() => toggleUserStatus(u)}/>
                        )}
                  
                        {!loadingMe && isAdmin && !isMeRow && (
                          <IconDelete onClick={() => openModal({message: `¿Quieres eliminar al usuario "${u.name}"?\nRol: ${getRoleLabel(u)}\nEsta acción no se podrá revertir.`, onConfirm: () => handleDeleteUser(u),})}/>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {!loading && meta.total > 0 && ( <Paginator page={page} lastPage={meta.last_page} setPage={setPage} /> )}
